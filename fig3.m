...
%% 1. CÀI ĐẶT THÔNG SỐ QUÉT HỆ THỐNG
Nd      = 1e17;  
B_field = 0;     
Nstate  = 4;     % Dù hàm trả về 15 mức, ta vẫn chỉ lấy 4 mức thấp nhất để vẽ
...
for step = 1:length(F_array)
    ...
    for outer_iter = 1:max_outer
        ...
        for inner_iter = 1:max_inner
            ...
            % Gọi schrodinger.m (Bây giờ nó trả về cực nhanh ma trận 15x1)
    ...
    % Trích xuất 4 mức thấp nhất trong số 15 mức để lưu vào kết quả
    E_results(step, :) = E_joule(1:Nstate) / e * 1000;
    fprintf('Xong! (Lặp: %d outer)\n', outer_iter);
end

%% 8. XỬ LÝ DỮ LIỆU & VẼ ĐỒ THỊ
F_kVcm = F_array / 1e5; 
