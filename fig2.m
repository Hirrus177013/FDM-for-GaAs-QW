...
%% 1. CÀI ĐẶT 4 THÔNG SỐ HỆ THỐNG
Nd      = 1e17;   % Lượng donor trên m^2 (đặt 0 nếu không pha tạp)
B_field = 0;      % Từ trường B (Tesla)
F_field = 0;    % Điện trường F (V/m) - Chạy 1 điểm tĩnh
Nstate  = 4;      % Chỉ lấy 4 mức thấp nhất để in ra và vẽ đồ thị
...
eps  = 12.58;            % Hằng số điện môi tương đối
...
for outer_iter = 1:max_outer
    ...
    for inner_iter = 1:max_inner
        ...
        % Giải phương trình Schrödinger (Hàm này giờ tự động nhả ra đúng 6 mức)
        [E_raw, psi] = schrodinger(N, b_term, f_term, vc, vh, dz);

        % Phục hồi đơn vị năng lượng
        E_joule = E_raw * (hbar^2 / (2 * mstar));

        % Tính mật độ electron n(z) từ 6 mức
        if Nd == 0
            n = zeros(N, 1);
        else
...
% Vòng lặp lấy đúng Nstate (4) mức đầu tiên từ mảng 6 mức để hiển thị
for i = 1:Nstate
    fprintf('  E%d = %.4f meV\n', i, E_meV(i));
end
...
% Chỉ lấy 4 mức năng lượng đầu tiên để vẽ và xác định scale đồ thị
