...
% (Bản cập nhật tương thích với ma trận tối ưu 15 mức năng lượng)
% =========================================================================
clear; clc; close all;
...
Nstate  = 4;      % Số mức năng lượng quan tâm sau cùng để xuất đồ thị
...
% --- Thành phần từ trường và điện trường ---
...
fprintf('\n=====================================================\n');
fprintf('  BẮT ĐẦU VÒNG LẶP TỰ NHẤT QUÁN (CHỈ GIẢI 15 MỨC E)\n');
fprintf('=====================================================\n');
...
for outer_iter = 1:max_outer
    ...
    % ===== INNER LOOP: hội tụ VH =====
    for inner_iter = 1:max_inner
        VH_old = VH;   

        vh = (2 * mstar / hbar^2) * VH;

        % Giải phương trình Schrödinger (Hiện tại hàm này trả về 15 mức thấp nhất)
        [E_raw, psi] = schrodinger(N, b_term, f_term, vc, vh, dz);

        % Phục hồi đơn vị năng lượng (Joule)
        E_joule = E_raw * (hbar^2 / (2 * mstar));

        % Tính mật độ electron n(z) từ 15 mức năng lượng đó
        if Nd == 0
            n = zeros(N, 1);
        else
...
%% 8. OUTPUT KẾT QUẢ
V_mag      = b_term  * (hbar^2 / (2 * mstar));   
V_elec     = f_term  * (hbar^2 / (2 * mstar));   
V_tot_meV  = (V_conf + VH + V_mag + V_elec) / e * 1000;  
...
% Vòng lặp lấy đúng Nstate (4) mức đầu tiên từ mảng 15 mức
for i = 1:Nstate
    fprintf('  E%d = %.4f meV\n', i, E_meV(i));
end
...
% Chỉ lấy 4 mức năng lượng đầu tiên để xác định scale đồ thị
